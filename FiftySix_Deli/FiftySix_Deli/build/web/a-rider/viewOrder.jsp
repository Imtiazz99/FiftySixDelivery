<%-- 
    Document   : viewOrder
    Created on : 31 Jan 2021, 3:13:37 pm
    Author     : TK
--%>
...
<html>
    ...
    <div class="sidebar">
        ...
        <div class="main-content">
            ...
            <main>
        <h1 style="margin-top:100px;">On-going Orders</h1>
            ...
            <c:forEach items= "${orders}" var= "orders" >
                <c:if test="${orders.statusID == 1 || orders.statusID == 2 || orders.statusID == 3}">
                 ...
                 <td><c:out value="${orders.totPayment}"/></td>
                    <td>
                <center>
                <c:if test="${orders.statusID == 1}">
                   <a href="/FiftySix_Deli/orderCustController?action=updatePreparing&order_id=<c:out value="${orders.orderid}"/>&rider_id=<%= session.getAttribute("rider_id")%>"><button class="buttons">Preparing</button></a>
                 </c:if> 
                <c:if test="${orders.statusID == 2}">
                   <a href="/FiftySix_Deli/orderCustController?action=updateDeliveryTime&order_id=<c:out value="${orders.orderid}"/>&rider_id=<%= session.getAttribute("rider_id")%>""><button class="buttons">Delivering</button></a>
                 </c:if> 
                 <c:if test="${orders.statusID == 3}">
                   <a href="/FiftySix_Deli/orderCustController?action=updateCompleteTime&order_id=<c:out value="${orders.orderid}"/>&rider_id=<%= session.getAttribute("rider_id")%>""><button class="buttons">Completed</button></a>
                 </c:if> 
                </center>
             </td>
             </c:if>
